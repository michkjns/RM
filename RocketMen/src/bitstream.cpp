...
public:
	...
	void writeBits(bool value, size_t numBits /* = 1 */) override;
	void writeBytes(char value, size_t numBytes /* = 1 */) override;
	...
	void ReadBytes(char* output, size_t numBytes /* = 1 */) override;
...
int BitStream_impl::readInt()
{
	int out;
	ReadBytes(reinterpret_cast<char*>(&out), sizeof(int));
	return out;
}
...
void BitStream_impl::writeBytes(char value, size_t numBytes)
{
	for (int writeByte = 0; writeByte < (int)numBytes; writeByte++)
	{
		if (m_writeBit == 0)
		{
			*m_writeData = value;
...
void BitStream_impl::writeBits(bool value, size_t numBits)
{
	//Don't overflow the buffer
	assert(m_writeTotalBytes < BUFFERLENGTH);
	if (m_writeTotalBytes >= BUFFERLENGTH) return /*false*/;

	int numBytes = (int)floorf((float)numBits / 8);
	if (numBytes >= 1)
	{
	...
	int stopAt = m_writeBit + static_cast<int>(numBits);
	for (m_writeBit; m_writeBit < stopAt; m_writeBit++)
	{
		if (m_writeBit == 8)
		{
			m_writeData++;
